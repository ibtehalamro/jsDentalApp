...
class DentalManager {
  ...
  addAppointmentFromUser() {
    ...
    const appointment = new Appointment(patientId, dateObject, time);
  ...
  listPatients = () => {
    ...
    for (let patient of patients) {
      const email =
        patient.getEmail() !== "" ? `email: ${patient.getEmail()}` : "";
      console.log(
        `id: ${patient.getId()} - name: ${patient.getName()} - age: ${patient.getAge()} - gender: ${patient.getGender()} - address: ${patient.getAddress()} - mobile number: ${patient.getMobileNumber()} - ${email}`
      );
    }
  };
  listAppointments = () => {
    ...
    for (let appointment of appointments) {
      console.log(
        `Appointment ID: ${appointment.getPatientId()} - Date: ${appointment.getDate()} - Time: ${appointment.getTime()}`
      );
    }
  };
  listTreatments = () => {
